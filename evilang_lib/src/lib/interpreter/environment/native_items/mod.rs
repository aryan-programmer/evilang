...
pub fn push_res_stack(
	env: &mut Environment,
	params: FunctionParameters
) -> ResultWithError<FunctionReturnValue> {
	env.global_scope.borrow_mut().res_stack.extend(params);
...
pub fn to_string(
	...
	Ok(
		PrimitiveValue::String(match params.first().unwrap() {
			...
			pv => {
				return Err(
					RuntimeError::InvalidArgumentsToFunction(
...
pub fn allocate_object(
	...
	if params.len() > 2 {
		return Err(
			(RuntimeError::InvalidNumberArgumentsToFunction {
