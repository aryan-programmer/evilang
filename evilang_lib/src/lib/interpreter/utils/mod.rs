...
pub fn expect_object(
	...
	return Ok(
		gc_clone(
			expect_object_fn(object.borrow().deref(), ||
				expr.map(|v| Descriptor::Expression(v.clone())).unwrap_or(Descriptor::None)
			)?
		)
	);
}

pub fn expect_object_fn<T>(object: &PrimitiveValue, desc_fn: T) -> ResultWithError<&GcPtrToObject>
	where T: Fn() -> Descriptor
{
...
{
	return Ok(
		if object.borrow().deref() == &PrimitiveValue::Null {
