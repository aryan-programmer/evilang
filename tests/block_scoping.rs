...
fn block_scope_failure_tests(input: &str, typ: Option<ErrorT>) -> TestRes {
	let inp_str = input.to_string();
	for (left, right) in BLOCK_OPTS.iter() {
		ensure_execution_fails(inp_str.replace("{", left).replace("}", right), typ.clone());
	}
}
...
"#,
		vec![PrimitiveValue::String("inner_x".into()), PrimitiveValue::Null]
	);
...
"#,
		vec![PrimitiveValue::String("inner_x".into()), PrimitiveValue::String("outer_x".into())]
	)
}
...
"#,
		vec![PrimitiveValue::String("z".into())]
	)
}
...
fn no_redeclaration() -> TestRes {
	...
	block_scope_failure_tests(
		r#"{let c; let c = 1;}"#,
		Some(ErrorT::CantRedeclareVariable("c".into()))
	);
}
...
fn no_access_of_undeclared_variables() -> TestRes {
	...
	block_scope_failure_tests(
		r#"{let v = b; let b;}"#,
		Some(ErrorT::CantAccessHoistedVariable("b".into()))
	);
}
