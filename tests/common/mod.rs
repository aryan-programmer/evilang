...
impl TestData {
	...
	pub fn expect_statements(mut self, expected: StatementList) -> Self {
		if let Some(results) = &self.statement_results {
			assert_eq!(
				expected.len(),
				results.len(),
				"Expected lengths of expected Statements list and expected results list to match"
			);
		}
	...
	pub fn expect_statements_and_results(
		mut self,
		...
		if let Some(parsed) = &self.parsed {
			...
			match parse(self.input.clone()) {
				Ok(parsed) => {
					...
					let Some(parsed_) = &self.parsed else {
						panic!();
					};
					parsed_.clone()
				}
		...
		for (stmt, expected_val) in parsed.iter().zip(results.iter()) {
			if let ExpressionStatement(expr) = stmt {
				...
				assert_eq!(
					&PrimitiveValue::Null,
					expected_val,
					"Expected expected value to be null for not expression statements"
				);
			}
		}
	}
	...
	pub fn check_stack_results_no_exec(&mut self, env: &mut Environment) -> TestRes {
		...
		assert_eq!(
			&env.global_scope.borrow().res_stack,
			stack,
			"Expected result stack values to match"
		);
	}
...
pub fn ensure_program(input: &str, expected: StatementList) -> TestRes {
	TestData::new(input.to_string()).expect_statements(expected).check_parsing()
}
...
pub fn ensure_program_statement_results(
	...
	TestData::new(input.to_string()).expect_statements_and_results(expected, results).check()
}

pub fn ensure_res_stack_matches(input: &str, results: Vec<PrimitiveValue>) -> TestRes {
	TestData::new(input.to_string()).expect_stack(results).check()
}
...
pub fn identifier_stmt(iden: &str) -> Statement {
	Identifier(iden.into()).consume_as_statement()
}

pub fn push_res_stack_stmt(val: Expression) -> Statement {
	ExpressionStatement(
		Expression::function_call(Identifier("push_res_stack".to_string()).into(), vec![val])
	)
}
