...
#[test]
fn equality() -> TestRes {
	test_expression_and_assignment("x == null;", BinaryExpression {
		operator: Equals,
...
fn inequality() -> TestRes {
	test_expression_and_assignment("x != false;", BinaryExpression {
		operator: NotEquals,
		left: Identifier("x".parse().unwrap()).into(),
...
fn complex_equality() -> TestRes {
	test_expression_and_assignment("x * y + 13 > 15 == true;", BinaryExpression {
		operator: Equals,
