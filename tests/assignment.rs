...
fn simple_assignment() -> TestRes {
	let mut env = Environment::new_from_primitives(
		HashMap::from([("x".into(), PrimitiveValue::integer(-1))]),
		DefaultResolver::new_box()
	).unwrap();
	TestData::new("x;x = 1;x;".to_string())
		.expect_statements_and_results(
			vec![
				identifier_stmt("x"),
				(AssignmentExpression {
					operator: Assignment,
			...
			vec![PrimitiveValue::integer(-1), PrimitiveValue::integer(1), PrimitiveValue::integer(1)]
		)
...
fn complex_assignment() -> TestRes {
	let mut env = Environment::new_from_primitives(
		HashMap::from([("x".into(), PrimitiveValue::integer(12))]),
		DefaultResolver::new_box()
	).unwrap();
	(
		{
...
fn chained_assignment() -> TestRes {
	...
	x;
	y;
	"#.to_string()).expect_statements_and_results(
			vec![
				identifier_stmt("x"),
				identifier_stmt("y"),
				(AssignmentExpression {
