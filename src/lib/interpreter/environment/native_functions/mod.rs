...
pub fn allocate_object(
	...
	let class_val = if let Some(v) = class_val_opt { v } else { get_object_superclass(env)? };
	let PrimitiveValue::Object(ref object_class) = class_val else {
		return Err(RuntimeError::ExpectedClassObject(class_val.into()).into());
	};
	let name = if let Some(PrimitiveValue::String(ref s)) = name_opt { s.clone() } else {
		INSTANCE_OF_.to_string() + &object_class.borrow().name
	};
	return Ok(PrimitiveValue::Object(RuntimeObject::allocate(gc_cell_clone(object_class), name)));
}
